![Tyro](https://res.cloudinary.com/roxlox/image/upload/v1653133921/hydra/hydra-trnsparent_jcsl4l.png)
...
- Create the `admin@hydra.project` superuser (password `hydra`) so you always have a token-ready account.
...
| `privileges(): Collection` | Privileges | Returns the unique collection of privileges inherited through the user's roles (pre-hydrated when `roles` is already loaded). |
| `hasPrivileges(array $privileges): bool` | Privileges | Ensures the user inherits *all* of the provided privilege slugs directly from their roles (no Sanctum token refresh needed). |
...
Override defaults in `config/hydra.php` to align with your app:

| Option | Description |
| --- | --- |
| `version` | Value returned by `/api/hydra/version`. |
...
Tyro registers the following API endpoints (prefixed by `hydra.route_prefix`, `api` by default):

- `GET /hydra`, `GET /hydra/version`
- `POST /login`
...
Wrap any route with the `hydra.log` middleware to capture request/response diagnostics inside `storage/logs/laravel.log`.

### Fine-grained protections for your own routes

Tyro exposes the exact middleware aliases it relies on (`ability`, `abilities`, `privilege`, `privileges`, `role`, `roles`, `hydra.log`, plus whichever `auth` guard you configure), so locking down your own endpoints feels identical to the built-in API.

#### Quick reference

| Middleware | When to use it | Example |
| --- | --- | --- |
| `auth:hydra.guard` | Ensures the request is authenticated via Sanctum (default) or your custom guard. | `auth:'.config('tyro.guard', 'sanctum')` |
...
| `role:comma,list` | Require *all* listed roles on the authenticated user (honours wildcard `*`). | `'role:admin,super-admin'` |
| `roles:comma,list` | Allow access when the user holds *any* of the listed roles (no token re-issue required). | `'roles:editor,admin'` |
| `privilege:comma,list` | Require *all* listed privileges directly against the authenticated user (no need to reissue tokens). | `'privilege:reports.run,export.generate'` |
| `privileges:comma,list` | Allow access when the user has *any* of the listed privileges, checked in real time. | `'privileges:billing.view,reports.run'` |
| `hydra.log` | Log request/response pairs for auditing privileged routes. | `'tyro.log'` |
...
2. **(Optional) Group abilities in config**
	Publish `config/hydra.php` and add helper buckets so you do not repeat strings:
...
5. **Audit sensitive flows**
	Chain `hydra.log` when you want Laravel's log to record payloads and responses for forensic review:
...
Any token containing *either* the `admin` role slug, the `super-admin` role slug, or the `reports.run` privilege gets through. Everyone else receives 403.
...
| `tyro:flush-roles` | Truncate the roles + pivot tables without re-seeding them. |
...
| `tyro:publish-config` | Drop `config/hydra.php` into your app (respects `--force`). |
...
curl -X POST http://localhost/api/login \
	-H "Accept: application/json" \
	-H "Content-Type: application/json" \
	-d '{"email":"admin@hydra.project","password":"hydra"}'
```
...
# Update my profile (requires ability list in `hydra.abilities.user_update`)
...
class ProfileController
{
	...
	{
		$roleSlugs = $request->user()->hydraRoleSlugs();
...
class ApiTokenController
{
	...
	{
		$privileges = $request->user()->hydraPrivilegeSlugs();
...
- `database/migrations/*` creates the `roles`, `user_roles`, `privileges`, and `privilege_role` tables (configurable via `config/hydra.php`).
